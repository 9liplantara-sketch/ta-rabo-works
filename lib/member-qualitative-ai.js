...
5. self_report の supports evidence は daily_report または intake_response であること。knowledge_record を含む候補は self_report にしない。
5b. intake_response は本人の Form 自由記述（self_report）。観察事実・確定性格・客観事実へ勝手に昇格させない。
...
export function validateSelfReportProvenance(epistemicType, evidence, _sourceMetaByKey = null) {
  ...
  for (const ev of supports) {
    ...
    if (kind === 'intake_response') continue;
    if (kind === 'knowledge_record') {
      errors.push('self_report forbidden: knowledge_record supports evidence not allowed in Phase M3');
    } else {
      errors.push(
        `self_report supports evidence must be daily_report or intake_response (got ${kind || 'unknown'})`,
      );
    }
  }
...
export function buildStep1Prompt(sources) {
  ...
  return `あなたは研究室の教員向けアシスタントです。以下は学生の日報・研究室記録、および Form 自由記述（intake_response / self_report）です（定量アンケートの尺度スコアはまだ与えません）。

${AI_SYSTEM_RULES}


source_kind の扱い:
- daily_report / knowledge_record: 行動・文脈の記録（観察候補の主材料）
- intake_response: 本人の自己申告（self_report）。観察事実・確定性格・客観事実へ勝手に昇格させない
  ...
  "observations": [
    {
      ...
      "evidence": [
        { "source_kind": "daily_report|knowledge_record|intake_response", "source_id": "id", "evidence_role": "supports" }
      ]
    }
  ]
}
...
export function buildStep2Prompt(step1Observations, existingProfile, psychScores) {
  return `Step1で日報・記録・intake self_report から得た観察:
...
定量スコアに合わせて日報や intake を都合よく解釈しないこと。観察・自己申告を主とし、スコアは照合用参考値。intake_response を客観事実へ昇格させない。
