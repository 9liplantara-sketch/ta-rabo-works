...
export async function claimNextWorkerJob(workerId) {
  ...
  const leaseSeconds = WORKER_LEASE_SECONDS;
  ...
  const window = {
    ...
    fromDate: String(run.windowStart).slice(0, 10),
    toDate: String(run.windowEnd).slice(0, 10),
  };
...
export async function submitWorkerJobResult(body = {}) {
  ...
  const window = {
    ...
    fromDate: String(run.window_start).slice(0, 10),
    toDate: String(run.window_end).slice(0, 10),
  };
