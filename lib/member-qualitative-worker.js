...
import {
  ...
  listPsychAssessmentRowsForServerAnalysis,
} from './psych-assessments.js';
import { selectPsychAssessmentForAnalysis } from './member-analysis-assessment-selection.js';
import { assertLocalAiAnalysisConsentOrThrow } from './member-local-ai-consent.js';
import { INTAKE_SOURCE_KIND } from './member-qualitative-intake-sources.js';
...
export function formatWorkerPayloadSources(sources) {
  return (sources || []).map((s) => {
    let title;
    if (s.sourceKind === 'daily_report') title = '日報';
    else if (s.sourceKind === INTAKE_SOURCE_KIND) title = s.title || s.itemId || 'intake_response';
    else title = String(s.sourceType || 'knowledge_record');
    ...
    if (s.sourceKind === INTAKE_SOURCE_KIND) {
      out.item_id = s.itemId || null;
      out.question = s.question || s.title || null;
      out.epistemic_type = s.epistemicType || 'self_report';
      out.scope = s.scope || null;
      out.question_version = s.questionVersion || null;
    }
    return out;
  });
}
...
async function getLatestPsychForWorker(studentId) {
  const rows = await listPsychAssessmentRowsForServerAnalysis({ studentId });
  const { assessment } = selectPsychAssessmentForAnalysis(rows);
  return assessment ? mapPsychAssessmentForClient(assessment) : null;
}
...
export async function fetchAnalysisContextForStudent(studentId, window) {
  ...
  const psych = bundle.psych || await getLatestPsychForWorker(studentId);
...
export async function enqueueLocalWorkerAnalysis(user, studentId, windowInput = {}) {
  ...
  await assertLocalAiAnalysisConsentOrThrow(studentId, { provider: 'local_worker' });
...
/** Queue provider stored on member_analysis_runs.model_provider at enqueue time */
export const LOCAL_WORKER_RUN_PROVIDER = 'local_worker';

export function isValidAnalysisRunId(runId) {
  return RUN_ID_UUID_RE.test(String(runId || '').trim());
}

/**
 * Pure claim selection rules (mirrors SQL). Used by verifies; FIFO vs run-specific.
 * @param {Array<{id:string,status:string,model_provider?:string,provider?:string,created_at?:string|Date,lease_expires_at?:string|Date|null,attempt_count?:number}>} runs
 * @param {{ runId?: string, now?: Date, maxAttempts?: number }} options
 */
export function selectClaimCandidate(runs, options = {}) {
  const list = Array.isArray(runs) ? runs : [];
  const runId = String(options.runId || '').trim();
  const now = options.now instanceof Date ? options.now : new Date();
  const maxAttempts = Number.isFinite(options.maxAttempts) ? options.maxAttempts : WORKER_MAX_ATTEMPTS;

  if (runId) {
    if (!isValidAnalysisRunId(runId)) {
      return { selectedId: null, reason: 'invalid_run_id' };
    }
    const run = list.find((r) => String(r.id) === runId);
    if (!run) return { selectedId: null, reason: 'run_not_found', runId };
    const provider = String(run.model_provider || run.provider || '');
    if (provider !== LOCAL_WORKER_RUN_PROVIDER) {
      return { selectedId: null, reason: 'provider_mismatch', runId };
    }
    if (String(run.status) !== 'pending') {
      return { selectedId: null, reason: 'run_not_pending', runId, status: String(run.status) };
    }
    return { selectedId: run.id, reason: null, runId };
  }
  ...
  const eligible = list
    .filter((r) => {
      ...
      const lease = r.lease_expires_at != null ? new Date(r.lease_expires_at) : null;
      const attempts = Number(r.attempt_count || 0);
      return lease && !Number.isNaN(lease.getTime()) && lease < now && attempts < maxAttempts;
    })
    .sort((a, b) => {
  ...
  if (!eligible.length) return { selectedId: null, reason: null };
  return { selectedId: eligible[0].id, reason: null };
}

/**
 * Diagnose why a run-specific claim returned no row (fail-closed; never selects another run).
 */
export function diagnoseRunSpecificClaimSkip(runRow, runId) {
  const id = String(runId || '').trim();
  if (!isValidAnalysisRunId(id)) {
    return { skipped: 'invalid_run_id', run_id: id || null };
  }
  if (!runRow) {
    return { skipped: 'run_not_found', run_id: id };
  }
  const provider = String(runRow.model_provider || runRow.provider || '');
  if (provider !== LOCAL_WORKER_RUN_PROVIDER) {
    return { skipped: 'provider_mismatch', run_id: id };
  }
  if (String(runRow.status) !== 'pending') {
    return { skipped: 'run_not_pending', run_id: id, status: String(runRow.status) };
  }
  return { skipped: 'run_not_claimable', run_id: id };
}

async function finalizeClaimedWorkerJob(sql, runRow, claimToken) {
  const run = mapRunRow(runRow);
  const window = buildAnalysisWindowFromRun(runRow);
  const ctx = await fetchAnalysisContextForStudent(run.studentId, window);

  try {
    await assertLocalAiAnalysisConsentOrThrow(run.studentId, { provider: 'local_worker' });
  } catch (consentErr) {
    await sql`
      UPDATE member_analysis_runs
      SET status = 'failed',
          error_text = ${String(consentErr.code || consentErr.message || 'local_ai_consent_required').slice(0, 2000)},
          completed_at = NOW()
      WHERE id = ${run.id}::uuid
    `;
    return { ok: true, job: null, skipped: consentErr.code || 'local_ai_consent_required', run_id: run.id };
  }
...
async function claimSpecificWorkerJob(sql, wid, runId) {
  const id = String(runId || '').trim();
  if (!isValidAnalysisRunId(id)) {
    return { ok: true, job: null, skipped: 'invalid_run_id', run_id: id || null };
  }
  ...
  const rows = await sql`
    WITH candidate AS (
      ...
      WHERE id = ${id}::uuid
        AND status = 'pending'
        AND model_provider = ${LOCAL_WORKER_RUN_PROVIDER}
  ...
  if (!rows.length) {
    const existing = await sql`
      SELECT id, status, model_provider
      FROM member_analysis_runs
      WHERE id = ${id}::uuid
      LIMIT 1
    `;
    const diag = diagnoseRunSpecificClaimSkip(existing[0] || null, id);
    return { ok: true, job: null, ...diag };
  }

  return finalizeClaimedWorkerJob(sql, rows[0], claimToken);
}

/**
 * Claim next FIFO job, or a single run when options.runId is set.
 * Run-specific mode never considers other pending runs.
 */
export async function claimNextWorkerJob(workerId, options = {}) {
  ...
  if (runId) {
    return claimSpecificWorkerJob(sql, wid, runId);
  }
