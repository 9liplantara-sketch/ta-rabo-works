...
export async function syncPsychAssessmentBatch({
  ...
  const isV3 = isV3QuestionnaireVersion(qVersion);

  // v1 採点には header mapping が必要。v3 は Phase 2 で scoring skip。
  ...
  for (const item of responses) {
    ...
    try {
      ...
      if (isV3) {
        // Phase 2: v3 は raw_answers + item_answers のみ保存。既存 v1 scorer は流さない。
        scoringVersion = SCORING_VERSION_V3_DEFERRED;
        scores = {};
        warnings = undefined;
      } else {
      ...
      results.push({
        ...
        scoring_deferred: isV3 || undefined,
        warnings,
      });
    } catch (e) {
      failed += 1;
