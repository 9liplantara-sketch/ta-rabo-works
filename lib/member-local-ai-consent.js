...
import { getDb, findStudentById } from './db.js';

export const LOCAL_AI_ANALYSIS_PURPOSE = 'local_ai_analysis';

/** students.id / consent student_id に要求する UUID 形式（ハイフン付き） */
const STUDENT_ID_UUID_RE =
...
/** @type {((studentId: string) => Promise<object | null> | object | null) | object | null | undefined} */
let studentLookupOverrideForTests = undefined;
...
/** @param {((studentId: string) => Promise<object | null> | object | null) | object | null | undefined} fn */
export function __testSetConsentStudentLookup(fn) {
  studentLookupOverrideForTests = fn;
}

export function __testResetConsentStudentLookup() {
  studentLookupOverrideForTests = undefined;
}

export function isValidStudentIdUuid(value) {
  return STUDENT_ID_UUID_RE.test(String(value || '').trim());
}

/**
 * consent-record / consent-withdraw 用: DB write 前に student 存在を fail-closed 確認。
 * FK violation を validation 代わりに使わない。
 */
export async function assertConsentStudentExistsOrThrow(studentId) {
  ...
  if (!isValidStudentIdUuid(sid)) {
    const err = new Error('invalid_student_id');
    err.status = 400;
  ...
  let student;
  if (studentLookupOverrideForTests !== undefined) {
    student =
      typeof studentLookupOverrideForTests === 'function'
        ? await studentLookupOverrideForTests(sid)
        : studentLookupOverrideForTests;
  } else {
    student = await findStudentById(sid);
  }
...
export async function recordConsentEpisode(input) {
  ...
  // DB write / FK に頼る前に student を明示確認（invalid → 400, missing → 404）
...
export async function withdrawConsentEpisode(input) {
  ...
  // UPDATE 前に student を明示確認（invalid → 400, missing → 404）
  await assertConsentStudentExistsOrThrow(studentId);
