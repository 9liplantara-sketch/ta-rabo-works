...
/** psych_assessments.id 部分に要求する UUID（ハイフン付き） */
const ASSESSMENT_ID_UUID_RE =
  /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

/**
 * Evidence / provenance 用: `<assessmentId>:<itemId>`
 * assessmentId は UUID。invalid は null（throw せず resolver 側で inaccessible）。
 * @param {string} sourceId
 * @returns {{ assessmentId: string, itemId: string } | null}
 */
export function parseIntakeEvidenceSourceId(sourceId) {
  ...
  if (!ASSESSMENT_ID_UUID_RE.test(assessmentId)) return null;
  return { assessmentId, itemId };
}

/**
 * 汎用 split（AI source builder 等）。evidence ownership には parseIntakeEvidenceSourceId を使う。
 * @param {string} sourceId
 * @returns {{ assessmentId: string, itemId: string } | null}
 */
