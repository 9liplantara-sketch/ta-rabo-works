...
/**
 * MEMBER 分析対象: id あり · is_active !== false（role 不問）
 */
export function isActiveMemberAnalysisMember(member) {
  const id = String(member?.id || '').trim();
  return !!id && member?.is_active !== false;
}

/** GET /api/students → MEMBER option 候補（active members · role 不問） */
