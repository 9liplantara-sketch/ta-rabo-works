...
import { INTAKE_SOURCE_KIND, parseIntakeEvidenceSourceId } from './member-qualitative-intake-sources.js';
...
async function enrichItemsWithEvidenceAccessibility(items, user) {
  ...
  for (const item of items) {
    ...
    const expectedStudentId = item.student_id;
    const evidence = [];
    for (const ev of evRows) {
      const accessible = await isEvidenceSourceAccessible(
        ev.source_kind,
        ev.source_id,
        user,
        expectedStudentId,
...
export async function runQualitativeAnalysis(user, studentId, windowInput = {}) {
  ...
  if (isQueueBasedProvider(provider)) {
    await assertLocalAiAnalysisConsentOrThrow(studentId, { provider });
  ...
  try {
    ...
    const psych = psychFromSources || await getLatestPsychAssessmentForStudent(studentId);
...
export async function getEvidenceDetailForItem(user, itemId) {
  ...
  const itemRows = await sql`
    ...
    LIMIT 1
  `;
  const profileItem = itemRows[0];
  if (!profileItem) return [];

  const expectedStudentId = profileItem.student_id;
  const evRows = await sql`
    SELECT id, profile_item_id, source_kind, source_id, evidence_role, observed_at
    FROM member_profile_evidence
    WHERE profile_item_id = ${itemId}::uuid
  ...
  for (const ev of evRows) {
    ...
    if (ev.source_kind === 'daily_report') {
      const dr = await sql`
        ...
        FROM daily_reports
        WHERE id = ${ev.source_id}
          AND student_id = ${expectedStudentId}::uuid
        LIMIT 1
      `;
      const row = dr[0];
      if (!isDailyEvidenceOwnedAndEligible(row, expectedStudentId)) {
        details.push(inaccessible);
        continue;
      }
    ...
    if (ev.source_kind === 'knowledge_record') {
      const kr = await sql`
        SELECT kr.id, kr.record_type, kr.title, kr.occurred_at, kr.session_key, kr.body_text, kr.visibility,
               kr.summary_text, kr.decisions_text, kr.next_actions_text, kr.created_at, kr.updated_at, kr.created_by
        FROM knowledge_records kr
        INNER JOIN knowledge_record_participants krp ON krp.record_id = kr.id
        WHERE kr.id = ${ev.source_id}
      ...
      const mapped = mapKnowledgeRecordRow(row, parts.get(row.id) || []);
      if (!isKnowledgeEvidenceOwnedAndViewable(mapped, user, expectedStudentId)) {
        details.push(inaccessible);
        continue;
      }
      const vm = mapKnowledgeRecordToSource(mapped);
      details.push({
        sourceKind: ev.source_kind,
        sourceId: ev.source_id,
        evidenceRole: ev.evidence_role,
    ...
    if (ev.source_kind === INTAKE_SOURCE_KIND) {
      ...
      const pa = await sql`
        SELECT id, student_id, item_answers, answered_at
        FROM psych_assessments
        WHERE id = ${parsed.assessmentId}::uuid
          AND student_id = ${expectedStudentId}::uuid
        LIMIT 1
      `;
      const resolved = resolveIntakeEvidenceDetailFromAssessment({
        sourceId: ev.source_id,
        assessment: pa[0] || null,
        expectedStudentId,
        user,
      });
      if (!resolved.accessible || !resolved.detail) {
        details.push(inaccessible);
        continue;
      }
      details.push({
