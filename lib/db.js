...
import { resolveCreateLoginEnabled } from './student-lifecycle.js';
...
let _findStudentByEmailOverride = null;

export function __testSetFindStudentByEmail(fn) {
  _findStudentByEmailOverride = fn;
}

export function __testResetFindStudentByEmail() {
  _findStudentByEmailOverride = null;
}

export async function findStudentByEmail(email) {
  if (_findStudentByEmailOverride) return _findStudentByEmailOverride(email);
  const sql = getDb();
...
export async function createStudent({ name, email = null, role = 'student', displayName = null, note = null, iconColor = null, loginEnabled = false }) {
  ...
  const canLogin = resolveCreateLoginEnabled(normalizedEmail, loginEnabled);
