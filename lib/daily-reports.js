import { getDb, findStudentByEmail } from './db.js';
import { getPublicDisplayName, resolveStudentIdentity } from './auth.js';

export const VALID_VISIBILITY = ['private', 'lab', 'public'];
const VALID_ATTACHMENT_TYPES = ['image', 'pdf', 'video', 'other'];
const MAX_ATTACHMENTS = 20;
const SESSION_KEY_MAX_LEN = 200;

/** session_key: null/空→null、trim保存、異常に長い値のみ拒否 */
export function parseSessionKey(raw) {
  if (raw === undefined || raw === null) return { ok: true, value: null };
  const s = String(raw).trim();
  if (!s) return { ok: true, value: null };
  if (s.length > SESSION_KEY_MAX_LEN) {
    return { ok: false, error: `session_key は ${SESSION_KEY_MAX_LEN} 文字以内にしてください` };
  }
...
/** 日報の本人判定: student_id がある行は ID のみ。legacy（student_id なし）のみ email fallback */
export function reportOwnedByUser(user, row) {
  if (!user || !row) return false;
  const rowSid = row.student_id;
  if (rowSid != null && rowSid !== '') {
    if (!user.studentId) return false;
    return String(user.studentId) === String(rowSid);
  }
...
export function canViewReport(user, row) {
  ...
  if (reportOwnedByUser(user, row)) return true;
...
export function canEditReport(user, row) {
  if (!row) return false;
  if (user.role === 'admin') return true;
  return reportOwnedByUser(user, row);
}
...
export async function listDailyReports(user, params) {
  const { view, limit, offset, q, from, to, project, studentId, sessionKey } = params;
  const sql = getDb();
...
/** POST 時の student フィールド決定（insert 前）。student は body 由来 identity を無視。 */
export async function resolveDailyReportStudentFields(user, body) {
  let studentEmail = user.email;
  let studentName;
  let studentId = user.studentId || user.id || null;
  ...
  if (user.role === 'admin' && body.student_email) {
    ...
    const dbStudent = await resolveStudentIdentity(user);
    studentEmail = dbStudent.email;
    studentName = dbStudent.display_name || dbStudent.name;
    studentId = dbStudent.id;
  }

  return { studentEmail, studentName, studentId };
}
...
export async function createDailyReport(user, body) {
  ...
  if (!skResult.ok) {
    const err = new Error(skResult.error);
    err.status = 400;
    throw err;
  }

  const { studentEmail, studentName, studentId } = await resolveDailyReportStudentFields(user, body);
  ...
  const rows = await sql`
    INSERT INTO daily_reports (
      ...
      session_key, drive_link, attachments, time_spent, work_location, visibility
    ) VALUES (
      ${reportDate},
      ${studentId},
      ${studentName},
...
export async function updateDailyReport(user, id, body) {
  ...
  let sessionKeyValue = existing.session_key ?? null;
  if (body.session_key !== undefined || body.sessionKey !== undefined) {
    ...
    sessionKeyValue = skResult.value;
  }
