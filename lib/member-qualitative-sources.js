 ...
 * Phase 6A: semantic v3 Form 自由記述を intake_response として追加（本文は正本参照）。
 */
...
/**
 * server-only: 選択 assessment + intake sources（item_answers は client へ返さない）
 */
export async function resolvePsychAndIntakeSourcesForAnalysis(studentId) {
  const rows = await listPsychAssessmentRowsForServerAnalysis({ studentId });
  const { assessment, selection } = selectPsychAssessmentForAnalysis(rows);
  if (!assessment) {
    return {
      psych: null,
      selection,
  ...
  const psych = mapPsychAssessmentForClient(assessment);
  let intakeSources = [];
  ...
  if (selection === 'current_semantic') {
    ...
    intakeSources = built.sources;
    intakeResponseCount = built.intakeResponseCount;
  }
...
export async function fetchAnalysisSourcesForStudent(user, studentId, window) {
  ...
  const { psych, selection, intakeSources, intakeResponseCount } =
    await resolvePsychAndIntakeSourcesForAnalysis(studentId);
  ...
  return (sources || []).map((s) => {
    ...
    if (s.sourceKind === INTAKE_SOURCE_KIND) {
      return {
        ...
        question: s.question || s.title || null,
        epistemic_type: s.epistemicType || 'self_report',
        scope: s.scope || null,
        question_version: s.questionVersion || null,
      };
    }
    return base;
  });
...
export function buildSourceMetaMap(sources) {
  ...
  for (const s of sources || []) {
    ...
    map.set(`${s.sourceKind}:${s.sourceId}`, {
      ...
      epistemicType: s.epistemicType || null,
    });
  }
  return map;
}
