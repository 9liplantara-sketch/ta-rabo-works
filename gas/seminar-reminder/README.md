...
## session_key（Phase 2a Step 1）

研究会の各回に不変 ID（例: `seminar_2026_001`）を付与します。日程を変更しても **session_key は変えません**。

### シート列

1行目ヘッダー末尾に **`session_key`** 列を追加（メニュー **研究会リマインド → session_key 列を追加** でも可）。

### 初回（既存行の一括採番）

1. Apps Script に最新 `Code.gs` を反映し **ウェブアプリを再デプロイ**
2. スプレッドシートで **研究会リマインド → session_key を一括採番**
3. 空欄行のみ、**シート上の行順**で `seminar_YYYY_NNN` が付与されます（既存セルは上書きしません）

### 新規行

- 日付を入力すると **onEdit** で空欄行に自動採番（その年の既存最大番号 + 1）
- 手入力も可（形式: `seminar_2026_001`）。既存値は自動処理で上書きしません

### JSON

`/api/seminar-schedule` の各要素に `session_key` が含まれます（未採番時は `null`）。既存の研究会表示・LINE リマインドは `session_key` なしでも動作します。
